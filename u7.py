...
ki = VIPRO.LINE()
#ki.login(qr=True)
ki.login(token='EtSDJ5uVw6l8gv3qkojf.0Gxc/M3RVqR4nYkPcDjtZW.7eafq5FprOOSv4RkXIdClQ77AppaFYwntQWXMYuM0C4=')
ki.loginResult()
print "Ki-Login Success\n"

kk = VIPRO.LINE()
#kk.login(qr=True)
kk.login(token='EtOm4WgkN3Cpx38D2KN6.u5E0qHumGolsyebggFHFzG.JVLjq58HW8zbIacp/Fim3hohXCrpULuFZV+77cDrtHc=')
kk.loginResult()
print "Kk-Login Success\n"

kc = VIPRO.LINE()
#kc.login(qr=True)
kc.login(token='EtEqe3j7vp7mQynO2sta.SD28EqyjtLBjPT144PdcQG.u5qmrGAiOGeThdVinnQyFhoBIpxDVMERF9FZsDacEHM=')
kc.loginResult()
print "Kc-Login Success\n"

kr = VIPRO.LINE()
#kr.login(qr=True)
kr.login(token='Et1lmsANFvgB7dv60aH8.BQIP++PfVBkYt2USSm1tMa.2Ml9J7gxoZH4K/ADo8evlX06mVZBx2GWoLq/RpCB7+8=')
kr.loginResult()
print "Kr-Login Success\n"

vipro = VIPRO.LINE() #(isi dengan akun utama fungsinya biar akun utama di kick langsung di invite bot)
#vipro.login(qr=True)
vipro.login(token='Et8D8zWMfLbqTwia6uHe.4uWnRTJU51yT+/CjXTyHBG.mtq7CFgj68HkOdSAp+1SgS6yS5OVHmHfj3JeBrbMCxQ=')
vipro.loginResult()
print "Vipro-Login Success\n"

km = VIPRO.LINE() #(bot ini di luar grup fungsinya jadi algojo kaya kicker siri)
#km.login(qr=True)
km.login(token='EtZpeYp45f1U76oVR2I5.s0GaOrjP6Vfn98NxODhvLq.oyDaqdAIKpabnw9LTOtekIkIQtCBznl4wq8MsEwQf9M=')
km.loginResult()
...
#		else:                    
		...
		if msg.from_ in Creator:
            ...
            elif msg.text in ["Yudi1 acc invite"]:
                if msg.from_ in admin:
                    gid = ki.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["yudi2 acc invite"]:
                if msg.from_ in admin:
                    gid = kk.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["yudi3 acc invite"]:
                if msg.from_ in admin:
                    gid = kc.getGroupIdsInvited()
                    _list = ""
                    for i in gid:
            ...
            elif msg.text in ["yudi4 acc invite"]:
                if msg.from_ in admin:
