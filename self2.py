    ...
    try:
	 ...
	 if wait["Ghost"] == True:
		...
		if msg.from_ in admin:
		    ...
		    cl.sendText(msg.to, "Khusus yudi")
...
#		    cl.sendText(msg.to,"Khusus yudi")
		...
		if wait["Simi"] == True: md+="╠➩✔️ Simisimi : On\n"
            ...
            elif msg.text in ["Viewseen","☆Checkpoint Checked☆","Ciduk","Cyduk"]:
	        lurkGroup = ""
		...
		if msg.from_ in admin:
		    ...
		    cl.sendText(msg.to,"sopo koen cok!")
		...
		cl.sendText(msg.to,"bapak hadir!!")
                ki.sendText(msg.to,"ibu hadir  \(ˆ▿ˆ)/")
                kk.sendText(msg.to,"kakak hadir  \(ˆ▿ˆ)/")
                kc.sendText(msg.to,"adik hadir  \(ˆ▿ˆ)/")
                kr.sendText(msg.to,"Hadiir Semua kan  \(ˆ▿ˆ)/")
		...
		cl.sendText(msg.to, "tingal ngopi sek...")
		...
		if msg.from_ in Creator:
		    cl.sendText(msg.to, "sek tak mati diluk...")
		    restart_program()
		    print "@Restart"
		else:
              ...
              if msg.from_ in Creator:
                ...
                for g in gs.members:
                    ...
                    if len(string.decode('utf-8')) <= 500:
                        ...
                        cl.sendText(msg.to,"wes")

            elif "cnkapten" in msg.text:
		if msg.from_ in Creator:
                    string = msg.text.replace("/cnkapten","yudi")
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc1","yudi 1")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc2","yudi 2")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc3","yudi 3")
                    if len(string.decode('utf-8')) <= 5000:
		...
		if msg.from_ in Creator:
                    string = msg.text.replace("/cntc4","yudi 4")
                    if len(string.decode('utf-8')) <= 5000:
                        profile = cl.getProfile()
                        profile.displayName = string
                        kr.updateProfile(profile)
